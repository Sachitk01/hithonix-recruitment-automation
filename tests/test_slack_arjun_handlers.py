...
import slack_arjun_handlers as handlers
...
async def test_arjun_dm_sends_ack_and_runs_pipeline(monkeypatch):
    ack_client = StubWebClient()
    monkeypatch.setattr(handlers, "_arjun_web_client", ack_client)
...
async def test_arjun_dm_pipeline_error_posts_message(monkeypatch, caplog):
    ...
    monkeypatch.setattr(handlers, "_run_arjun_pipeline", failing_pipeline)
    ...
    assert "arjun_dm_pipeline_crashed" in caplog.text
    assert slack_client.calls
    assert slack_client.calls[0]["channel"] == "D123"


async def test_arjun_ignores_bot_messages(monkeypatch):
    ack_client = StubWebClient()
    monkeypatch.setattr(handlers, "_arjun_web_client", ack_client)
...
async def test_arjun_ignores_non_message_events(monkeypatch):
    ack_client = StubWebClient()
    monkeypatch.setattr(handlers, "_arjun_web_client", ack_client)
...
async def test_arjun_channel_messages_do_not_trigger_dm_flow(monkeypatch):
    ack_client = StubWebClient()
    monkeypatch.setattr(handlers, "_arjun_web_client", ack_client)
